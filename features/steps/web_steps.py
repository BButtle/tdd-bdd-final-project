...
def step_impl(context, element_name):
    """Paste the copied value into a field"""
    element_id = ID_PREFIX + element_name.lower().replace(" ", "_")
    element = WebDriverWait(context.driver, context.wait_seconds).until(
        expected_conditions.presence_of_element_located((By.ID, element_id))
    )
...
def step_impl(context, message):
    """Check for expected flash message"""
    element = WebDriverWait(context.driver, context.wait_seconds).until(
...
def step_impl(context, value):
    """Check that a value appears in the search results"""
    ...
    assert value in results.text


@then('I should not see "{value}" in the results')
def step_impl(context, value):
    """Check that a value does not appear in the search results"""
    results = WebDriverWait(context.driver, context.wait_seconds).until(
        expected_conditions.presence_of_element_located((By.ID, "search_results"))
    )
    assert value not in results.text